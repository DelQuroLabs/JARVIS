...
export async function runAgent(messages: Msg[], opts: RunOptions): Promise<RunResult> {
  ...
  /* ---------------- RECALL: what we already know about this user ---------------- */
  const recall = opts.recall === false ? null : recallBlock(opts.ctx.memory.all(), prompt, { extra: opts.recallExtra });
  const system = recall ? `${systemFor(opts.mode)}\n\n${recall}` : systemFor(opts.mode);
  if (recall) step('think', `Recalled ${recall.split('\n').length - 1} memory item(s)`, 'persistent memory, added to the system prompt');
