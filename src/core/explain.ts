...
export const EXPLAINERS: Record<string, Explainer> = {
  '/app': {
    what: 'The launcher and status board. Every destination in the app is a button here, and the panels show weather, what is due, and what ran recently.',
    when: 'It is the first screen on open. Tap Rearrange to drag tiles into the order you actually use, or hide the ones you do not.',
  ...
  '/app/chat': {
    what: 'A conversation with the agent. It can call tools mid-answer, and every call is shown with its arguments and result.',
    ...
    limit: 'Without a model key it falls back to the offline reflex core, which handles arithmetic, memory and canned answers and says so rather than pretending to reason.',
  },
  '/app/agent': {
  ...
  '/app/memory': {
    what: 'Facts you have told the agent to remember, searchable and pinnable.',
    when: 'Anything it should carry between conversations: preferences, project context, names.',
    limit: 'Memory is injected into prompts, so a very large memory competes with your actual question for space.',
  },
  '/app/ideas': {
