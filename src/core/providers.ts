...
function toWire(messages: Msg[], spec: ProviderSpec, system?: string): Wire[] {
  ...
  if (!spec.multiTurn) {
    ...
    return last ? [{ role: 'user', content: last.content }] : [];
  }
  const wire: Wire[] = usable.map((m) => ({ role: m.role === 'tool' ? 'user' : m.role, content: m.content }));
  if (system && spec.systemRole) wire.unshift({ role: 'system', content: system });
  else if (system && wire.length) wire[0] = { role: 'user', content: `${system}\n\n---\n\n${wire[0].content}` };
  return wire;
}
