...
export default function Help() {
  ...
  const chapters: Chapter[] = [
    ...
    {
      id: 'assistant',
      title: 'JARVIS on Telegram',
      icon: 'paperplane',
      blurb: 'Text the bot from your phone; it runs real tools on your server.',
      to: '/app/assistant',
      cta: 'Open assistant',
      body: [
        {
          h: 'One chat, one account',
          p: 'Generate a six-character code in the app and send it to the bot. From then on that Telegram chat acts as you: it can log expenses, add contacts, schedule events, remember things and send email through your SMTP.',
        },
        {
          h: 'It runs on your server, not in this tab',
          p: 'The Telegram brain uses a model key on the server, so it answers even when the app is closed. Whatever it writes lands in the same per-user rows and shows up here on the next sync.',
        },
        {
          h: 'It learns you, and shows its work',
          p: 'After each exchange a background pass extracts durable facts: preferences, people, goals. They are recalled into later conversations and listed in full under Assistant → Memory, where you can trust, correct or forget any of them. Anything that looks like an injected instruction is quarantined rather than remembered.',
        },
        {
          h: 'It asks before it sends or deletes',
          p: 'Email is drafted for confirmation first, and deletes are confirmed too, unless you explicitly tell it to just do it. Voice notes are not transcribed yet; the bot says so rather than guessing.',
        },
      ],
    },
    {
