...
import { launchChips, routeLaunch } from '../../core/launch.ts';
...
export default function Home() {
  ...
  const chips = useMemo(
    () => launchChips({ weatherOn: app.settings.weatherOn, hasEvents: events.length > 0, daySeed, sparks: SPARKS }),
    [app.settings.weatherOn, events.length, daySeed],
  );
  const [cmd, setCmd] = useState('');
  const cmdRef = useRef<HTMLInputElement>(null);
  const [cmdHint, setCmdHint] = useState('');

  /** One line in, the right screen out. Nothing is sent on the user's behalf except the brief. */
  const launch = (raw: string) => {
    const r = routeLaunch(raw);
    haptic.light();
    if (r.kind === 'brief') {
      speakBrief();
      setCmd('');
    ...
    if (r.kind === 'navigate') {
      if (r.prompt) app.setPendingPrompt(r.prompt);
      navigate(r.path);
      return;
    }
    if (r.mode && r.mode !== app.settings.mode) app.setSettings({ mode: r.mode });
    app.setPendingPrompt(r.prompt);
    navigate('/app/chat');
  };
  // A line handed over on arrival (the palette's "Brief me") runs once, on
  // mount only, so the launcher's own hand-offs to other screens never loop back.
  useEffect(() => {
    const line = app.pendingPrompt;
    if (!line) return;
  ...
  const onCmdChange = (v: string) => {
    setCmd(v);
    if (!v.trim()) { setCmdHint(''); return; }
    const r = routeLaunch(v);
    setCmdHint(r.kind === 'brief' ? 'Brief' : r.label);
  };
  ...
  return (
    <Shell title="Dashboard" sub={`${mode.name} mode \u00b7 ${TOOLS.length} tools ready`}>
      <div className="dash">
        ...
        {/* ---------- greeting + the launcher: one line in, the right place out ---------- */}
        <header className="dash-hello">
          <span className="glow-ring">
            <Reactor size={42} busy={speaking} />
          </span>
          <div className="grow">
            <h1>{greeting.text}.</h1>
            <p className="muted" style={{ fontSize: '0.85rem', margin: '3px 0 0' }}>What can I do for you?</p>
          </div>
        </header>

        <form
          className="launcher"
          onSubmit={(e) => { e.preventDefault(); launch(cmd); }}
          role="search"
        ...
        >
          ...
          <input
            ...
            placeholder={'Ask, or say where to go: \u201cbrief me\u201d, \u201copen memory\u201d, \u201cadd expense 12 coffee\u201d'}
            autoComplete="off"
            autoCapitalize="sentences"
            enterKeyHint="go"
            aria-label="Ask JARVIS"
          />
          {cmdHint && <span className="launcher-hint" aria-live="polite">{cmdHint}</span>}
          <button type="submit" className="launcher-go" aria-label="Go" disabled={!cmd.trim()}>
            <Icon name="chevron" size={18} />
          </button>
        </form>
        <div className="row wrap" style={{ gap: 6, margin: '10px 0 14px' }}>
          {chips.map((c) => (
            <button
              key={c}
              type="button"
              className={`chip${c === 'Brief me' && speaking ? ' on' : ''}`}
              onClick={() => {
                if (c === 'Brief me' && speaking) { globalThis.speechSynthesis?.cancel(); setSpeaking(false); return; }
                launch(c);
              }}
            >
              {c === 'Brief me' && <Icon name={speaking ? 'stop' : 'mic'} size={12} />}
              {c === 'Brief me' && speaking ? 'Stop' : c}
            </button>
          ))}
        </div>

        {/* ---------- status strip ---------- */}
        <div className="strip" style={{ margin: '0 0 14px' }}>
