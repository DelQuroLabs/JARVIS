...
import { AskedStrip, Btn, Card, IconBtn, SectionTitle, useConfirm } from '../components.tsx';
...
export function useAllEvents(): CalEvent[] {
  ...
  return useMemo(
    () => [
      ...app.calendars.flatMap((c) => c.events),
      ...app.events.map((e): CalEvent => ({ uid: e.id, title: e.title, start: e.start, end: e.end, allDay: false, location: e.location, description: e.notes, recurring: false })),
    ].sort((a, b) => a.start - b.start),
    [app.calendars, app.events],
  );
}
...
export default function Calendar() {
  ...
  return (
    <Shell
      title="Calendar"
      sub={`${app.calendars.length} imported \u00b7 ${app.events.length} from JARVIS \u00b7 ${events.length} events`}
    ...
    >
      <input
        ...
        tabIndex={-1}
      />
      <AskedStrip pending={app.pendingPrompt} consume={() => app.setPendingPrompt('')} onAsk={(q) => { app.setPendingPrompt(q); navigate('/app/chat'); }} />

      {app.calendars.length === 0 && app.events.length === 0 && (
        <Card>
