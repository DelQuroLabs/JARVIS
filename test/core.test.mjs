...
/* ------------------------------------------------------------------ */
describe('loop recall', () => {
  test('the system prompt sent to the provider carries persistent memory', async () => {
    const realFetch = globalThis.fetch;
    let seenSystem = '';
    globalThis.fetch = async (_url, init) => {
      const body = JSON.parse(init.body);
      seenSystem = body.messages.find((m) => m.role === 'system')?.content ?? '';
      const payload = { choices: [{ delta: { content: 'Hello Dan.' } }] };
      return new Response(`data: ${JSON.stringify(payload)}\n\ndata: [DONE]\n\n`, { status: 200, headers: { 'Content-Type': 'text/event-stream' } });
    };
    try {
      const items = [];
      const memory = C.memoryStoreFrom(() => items, (n) => { items.length = 0; items.push(...n); });
      memory.add({ kind: 'fact', text: "The user's name is Dan", tags: [], source: 'learned' });
      const c = C.makeCtx({ fs: {}, onFsChange: () => {}, privacy: 'GUARDED', memory, approve: async () => true, serviceKeys: {} });
      const opts = { ctx: c, privacy: 'GUARDED', provider: { id: 'groq', key: 'gsk_test_key_long_enough' }, chain: [], allowKeyless: false, mode: C.modeOf('assist'), recallExtra: ['The user runs Coolify'] };
      const r = await C.runAgent([{ id: '1', role: 'user', content: 'Please greet me by name and mention my server', ts: Date.now() }], opts);
      assert.ok(seenSystem.includes('WHAT YOU REMEMBER'), 'recall block present');
      assert.ok(seenSystem.includes("The user's name is Dan"));
      assert.ok(seenSystem.includes('[learned] The user runs Coolify'));
      assert.ok(r.steps.some((s) => /Recalled/.test(s.label)));
      // private mode: no memory leaves the device
      seenSystem = '';
      await C.runAgent([{ id: '2', role: 'user', content: 'greet me', ts: Date.now() }], { ...opts, mode: C.modeOf('private'), recall: false });
      assert.ok(!seenSystem.includes('WHAT YOU REMEMBER'));
    } finally {
