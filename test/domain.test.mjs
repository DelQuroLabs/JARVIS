...
describe('modes and ideas', () => {
  ...
  test('every mode prompt = its spec body + the shared contract with itself marked current', () => {
    const shared = new Set();
    for (const m of C.MODES) {
      const p = C.systemFor(m);
      assert.ok(p.startsWith(C.MODE_PROMPTS[m.id]), `${m.id} body comes first`);
      assert.ok(p.includes(`\n${m.id} (current):`), `${m.id} marks itself current in the roster`);
      assert.equal((p.match(/^[a-z]+ \(current\):/gm) ?? []).length, 1, `${m.id} marks exactly one mode current`);
      for (const other of C.MODES) assert.ok(new RegExp(`^${other.id}( \\(current\\))?:`, 'm').test(p), `${m.id} roster lists ${other.id}`);
      assert.ok(p.includes('HANDOFF ETIQUETTE'), `${m.id} carries the shared contract`);
      assert.ok(p.includes(`Budget: ${m.maxSteps} step`), `${m.id} body states its budget (${m.maxSteps})`);
      assert.ok(p.includes(`${m.toolBudget} tool call`), `${m.id} body states its tool budget (${m.toolBudget})`);
      // The shared block must be byte-identical across modes apart from the (current) marker
      shared.add(p.slice(p.indexOf('You are JARVIS. The selected mode')).replace(/ \(current\)/, ''));
    }
    assert.equal(shared.size, 1, 'shared contract identical across all modes');
  });

  test('model-tier tags are extracted alongside switch tags', () => {
    const r = C.extractSwitch('Partial answer.\n\n1 state 2 blockers 3 next\n[[switch:build]] [[model:code|syntax precision]]', 'agent');
    assert.equal(r.hint.id, 'build');
    assert.deepEqual(r.model, { tier: 'code', reason: 'syntax precision' });
    assert.ok(!r.text.includes('[['));
    assert.equal(C.extractSwitch('x [[model:gpt5]]', 'agent').model, undefined, 'unknown tier ignored');
    // private never routes by tag, even if the model slips one in
    const pr = C.extractSwitch('x [[switch:agent]] [[model:reasoning]]', 'private');
    assert.equal(pr.hint, undefined); assert.equal(pr.model, undefined); assert.equal(pr.text, 'x');
    assert.deepEqual(C.MODEL_TIERS.length, 8);
  });

  test('switch tags are extracted, validated and stripped', () => {
    const r = C.extractSwitch('Here is a rough answer.\n\n[[switch:research|This needs sources]]', 'brief');
    assert.equal(r.text, 'Here is a rough answer.');
    assert.deepEqual(r.hint, { id: 'research', reason: 'This needs sources' });
    // reason falls back to the blurb
    assert.equal(C.extractSwitch('x [[switch:deep]]', 'brief').hint.reason, C.modeOf('deep').blurb);
    // unknown or same-mode tags are stripped but produce no hint
    assert.equal(C.extractSwitch('x [[switch:nope|y]]', 'brief').hint, undefined);
    assert.equal(C.extractSwitch('x [[switch:brief|y]]', 'brief').hint, undefined);
    assert.equal(C.extractSwitch('x [[switch:brief|y]]', 'brief').text, 'x');
    // plain text untouched
    assert.deepEqual(C.extractSwitch('no tag here', 'agent'), { text: 'no tag here' });
  });
...
describe('dashboard launcher', () => {
  test('brief, open <screen>, and bare screen names route without a model', () => {
    assert.equal(C.routeLaunch('brief me').kind, 'brief');
    assert.equal(C.routeLaunch("What's my day look like?").kind, 'brief');
    assert.deepEqual(C.routeLaunch('open memory'), { kind: 'navigate', path: '/app/memory', label: 'Memory' });
    assert.equal(C.routeLaunch('take me to the keys').path, '/app/providers');
    assert.equal(C.routeLaunch('settings').path, '/app/settings');
    assert.equal(C.routeLaunch('show me my skills').path, '/app/skills');
  });
  test('visual answers carry the question; ledger asks go to the assistant', () => {
    const w = C.routeLaunch('What should I wear today?');
    assert.equal(w.path, '/app/weather'); assert.equal(w.prompt, 'What should I wear today?');
    assert.equal(C.routeLaunch("what's next?").path, '/app/calendar');
    assert.equal(C.routeLaunch('add expense 12 coffee').path, '/app/assistant');
    assert.equal(C.routeLaunch('I spent $40 on gas').path, '/app/assistant');
  });
  test('mode hints ride along to chat; plain asks keep the current mode', () => {
    const r = C.routeLaunch('Find the latest sources on SQLite FTS5 ranking');
    assert.equal(r.kind, 'chat'); assert.equal(r.mode, 'research');
    assert.equal(C.routeLaunch('Review this function for edge cases').mode, 'build');
    assert.equal(C.routeLaunch('Draft three subject lines for a launch email').mode, 'writer');
    const plain = C.routeLaunch('Why is the sky blue?');
    assert.equal(plain.kind, 'chat'); assert.equal(plain.mode, undefined); assert.equal(plain.prompt, 'Why is the sky blue?');
    // a real question that merely contains a screen word stays in chat
    assert.equal(C.routeLaunch('Which tools do carpenters use most?').kind, 'chat');
    assert.equal(C.routeLaunch('   ').prompt, '');
  });
  test('chips are deterministic and start with the brief', () => {
    const a = C.launchChips({ weatherOn: true, hasEvents: true, daySeed: 3, sparks: ['x', 'y'] });
    assert.equal(a[0], 'Brief me'); assert.ok(a.includes('y')); assert.ok(a.length <= 5);
    assert.deepEqual(a, C.launchChips({ weatherOn: true, hasEvents: true, daySeed: 3, sparks: ['x', 'y'] }));
  });
});

describe('look at this: attachments', () => {
  const png = 'data:image/png;base64,iVBORw0KGgo=';
  test('data URLs split and size correctly; junk is rejected', () => {
    assert.deepEqual(C.splitDataUrl(png), { mime: 'image/png', data: 'iVBORw0KGgo=' });
    assert.equal(C.splitDataUrl('https://x/y.png'), null);
    assert.equal(C.dataUrlBytes(png), 8);
  });
  test('images are dropped before persistence and replaced by an honest marker', () => {
    const m = { id: 'm1', role: 'user', content: 'what is this', ts: 1, images: [png, png] };
    const out = C.stripImages(m);
    assert.equal(out.images, undefined);
    assert.deepEqual(out.attached, [{ kind: 'image', bytes: 8 }, { kind: 'image', bytes: 8 }]);
    assert.equal(out.content, 'what is this');
    const conv = { id: 'c', title: 't', created: 1, updated: 1, messages: [m, { id: 'm2', role: 'assistant', content: 'a cat', ts: 2 }] };
    const sc = C.stripConversationImages(conv);
    assert.ok(!JSON.stringify(sc).includes('base64'));
    assert.equal(C.stripConversationImages({ ...conv, messages: [conv.messages[1]] }).messages[0].content, 'a cat');
  });
  test('vision is only claimed for providers whose endpoints take inline images', () => {
    assert.ok(C.canSee('openai') && C.canSee('gemini') && C.canSee('anthropic'));
    assert.ok(!C.canSee('groq') && !C.canSee('reflex') && !C.canSee('pollinations'));
  });
  test('shared payloads become an editable prompt line', () => {
    assert.equal(C.sharePrompt({ title: 'A', text: 'b', url: 'https://c', images: [] }), 'A\nb\nhttps://c');
    assert.equal(C.sharePrompt({ images: [png] }), 'What is this?');
    assert.equal(C.sharePrompt({ images: [] }), '');
  });
});

describe('persistent memory: recall + learn', () => {
  const now = Date.now();
  const mem = [
    { id: 'a', kind: 'fact', text: "The user's name is Dan", tags: [], created: now - 1000 },
    { id: 'b', kind: 'preference', text: 'The user likes concise answers', tags: [], created: now - 86400000 * 30 },
    { id: 'c', kind: 'note', text: 'Coolify server lives at coolify.delquro.com', tags: [], created: now - 86400000 * 3 },
    { id: 'd', kind: 'note', text: 'Grandma prefers tea', tags: [], created: now - 86400000 * 300 },
    { id: 'e', kind: 'task', text: 'Migrate JARVIS deploy to Coolify', tags: [], created: now - 5000, pinned: true },
  ];
  test('recall always carries pinned + preferences, then what is relevant, capped', () => {
    const picked = C.selectRecall(mem, 'how do I redeploy on coolify?', { now });
    const ids = picked.map((m) => m.id);
    assert.ok(ids.includes('e') && ids.includes('b'), 'pinned and preference always');
    assert.ok(ids.includes('c'), 'relevant note');
    assert.ok(ids.indexOf('e') < ids.indexOf('c'), 'pinned first');
    const block = C.recallBlock(mem, 'coolify', { now, extra: ['The user has a Telegram bot'] });
    assert.ok(block.startsWith('WHAT YOU REMEMBER'));
    assert.ok(block.includes('[learned] The user has a Telegram bot'));
    assert.ok(block.length <= C.RECALL_MAX_CHARS + 200);
    assert.equal(C.recallBlock([], 'hi'), null);
  });
  test('rules extract durable first-person facts and skip questions/hypotheticals', () => {
    const got = C.extractFacts("Hi! My name is Dan and I live in Jersey City. I prefer concise answers. Always reply in British English. What if I lived in Paris?");
    const texts = got.map((g) => g.text);
    assert.ok(texts.includes("The user's name is Dan"), texts.join(' | '));
    assert.ok(texts.includes('The user lives in Jersey City'));
    assert.ok(texts.includes('The user likes concise answers'));
    assert.ok(texts.some((t) => t.startsWith('Standing instruction: reply in British English')));
    assert.ok(!texts.some((t) => t.includes('Paris')), 'hypothetical ignored');
    assert.deepEqual(C.extractFacts('Should I move to Boston?'), []);
    assert.deepEqual(C.extractFacts('/remember buy milk'), [], 'slash commands are handled elsewhere');
    assert.equal(C.extractFacts('remember that the wifi password is on the fridge')[0].kind, 'note');
  });
  test('dedupe and single-value slots', () => {
    const cands = [{ text: "The user's name is Dan", kind: 'fact', tags: [] }, { text: "The user's name is Daniel", kind: 'fact', tags: [] }];
    assert.equal(C.dedupeLearned(cands, mem).length, 1);
    assert.equal(C.slotOf("The user's name is Daniel"), 'name');
    assert.equal(C.slotOf('The user likes tea'), null);
  });
  test('model pass output is parsed defensively', () => {
    assert.deepEqual(C.parseLearned('Sure! ```json\n["The user works at DelQuro", 42, "x"]\n```'), ['The user works at DelQuro']);
    assert.deepEqual(C.parseLearned('nothing here'), []);
    assert.deepEqual(C.parseLearned('[]'), []);
  });
});
